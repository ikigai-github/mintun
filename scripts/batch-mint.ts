import { Data, Lucid, UTxO, applyParamsToScript, fromText } from 'https://deno.land/x/lucid@0.10.7/mod.ts';
...
import { NftMetadata, NftMetadataFile, NftShape, createReferenceData, makeNftUnit, makeReferenceUnit } from './cip68.ts';
...
export async function prepareBatchMintTx(lucid: Lucid, cache: BatchMintCache, mints: NftMint[]) {
  ...
  .attachSpendingValidator(stateValidator.policy)
  .collectFrom([stateReferenceUtxo], Data.void())
  .attachMintingPolicy(batchMint.policy)
  .mintAssets({ 
    ...assets.user, 
    ...assets.reference 
  }, Data.void())
...
function encodeMint(mint: NftMint): NftMint {
  ...
  const collection = metadata.collection ? fromText(metadata.collection) : null;
  const website = metadata.website ? fromText(metadata.website) : null;
  const twitter = metadata.twitter ? fromText(metadata.twitter) : null;
